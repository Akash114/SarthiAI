...
- **Traces:** W3C `traceparent` propagation on `/v1` requests; export OpenTelemetry traces from FastAPI in **staging/production** first; local dev may run a collector or no-op exporter until hardening work lands.

Mobile: defer full OTel until needed for perf work; Sentry performance monitoring is optional and subordinate to product analytics.

## Rationale

- Avoids blocking feature work on full distributed tracing while meeting the “structured + trace context” direction.
