...
def test_intervention_history_and_detail(client):
    ...
    hist_resp = test_client.get("/interventions/history", params={"user_id": str(user_id)})
    ...
    detail_resp = test_client.get(
        f"/interventions/history/{log_id}",
        params={"user_id": str(user_id)},
    )
    assert detail_resp.status_code == 200
    detail = detail_resp.json()
    assert detail["snapshot"]["slippage"]


def test_intervention_history_forbidden(client):
    test_client, session_factory = client
    ...
    resp = test_client.get(
        f"/interventions/history/{log.id}",
